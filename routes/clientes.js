...
router.patch("/:clienteId", (req, res) => {
    ClienteModel.findOneAndUpdate({"_id": req.params.clienteId}, {$set: req.body}, {runValidators: true}).then((result) => {
...
router.put("/:clienteId", (req, res) => {
    ClienteModel.replaceOne({_id: req.params.clienteId}, req.body, {runValidators: true}, (err, resp) => {
        if (err) {
            res.status(400).send(err);
        } else {
...
router.post("/login", (req, res) => {
    ClienteModel.findOne({correo: req.body.correo}).then(cliente => {
        if (!cliente || cliente.contrasena !== req.body.contrasena) return res.sendStatus(404);
