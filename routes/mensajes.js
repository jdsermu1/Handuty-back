...
router.patch("/:mensajeId", (req, res) => {
    MensajeModel.findOneAndUpdate({_id: req.params.mensajeId}, {$set: req.body}, {runValidators: true}, (err, resp) => {
        if (err) {
            res.status(400).send(err);
        } else {
...
router.put("/:mensajeId", (req, res) => {
    MensajeModel.replaceOne({_id: req.params.mensajeId}, req.body, {runValidators: true}, (err, resp) => {
        if (err) {
            res.status(400).send(err);
        } else {
            if (resp.nModified === 0) return res.sendStatus(404);
