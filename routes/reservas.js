...
router.put("/:id", function (req, res, next) {
    Reserva.replaceOne({"_id": req.params.id}, req.body, {runValidators: true}).then((result) => {
...
router.patch("/:id", function (req, res, next) {
    Reserva.findOneAndUpdate({"_id": req.params.id}, {$set: req.body}, {runValidators: true}).then((result) => {
        res.sendStatus(200);
    }).catch((err) => {
        return res.status(400).send(err)
    });
    ...
    Reserva.find({servicio: servicio}).then(reservas => {
        reservas.filter(reserva => reserva.fechaInicio >= new Date())
            .forEach(reserva => {
                ...
                if ((from <= fechaI && fechaI < to) || (from < fechaF && fechaF <= to)) {
